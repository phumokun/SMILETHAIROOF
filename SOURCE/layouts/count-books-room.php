<?php 
    ...
    if (isset($_GET['id'])) {
        ...
        $query_wi = "SELECT COUNT(status) as st_wi FROM books
                    WHERE status = 'waiting' AND books.ref_host = $id 
                    GROUP BY status";
        ...
        $query_in = "SELECT COUNT(status) as st_in FROM books
                    WHERE status = 'CheckIn' AND books.ref_host = $id  
                    GROUP BY status";
        ...
        $query_ou = "SELECT COUNT(status) as st_ou FROM books
                    WHERE status = 'CheckOut' AND books.ref_host = $id  
                    GROUP BY status";
        ...
        $query_ca = "SELECT COUNT(status) as st_ca FROM books
                    WHERE status = 'Cancel' AND books.ref_host = $id  
                    GROUP BY status";
